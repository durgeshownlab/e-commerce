...
try
{
    ...
    $sql="update orders set is_canceled=1, order_status='canceled', order_event='{$json_order_event_data}' where user_id={$_SESSION['user_id']} and order_id='{$order_id}' and is_deleted=0 ";
    $result=mysqli_query($conn, $sql);
    if($result)
    {
        echo 1;
